...
class User(models.Model):
    ...
    statistics = models.ForeignKey('UserStatistics', on_delete=models.SET_NULL, null=True, related_name="user_statistics")
    marker = models.ForeignKey('Marker', on_delete=models.SET_NULL, null=True, related_name="assigned_user")
...
class UserStatistics(models.Model):
    ...
    # Fields
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="player_statistics")
...
class Game(models.Model):
    ...
    gameState = models.CharField(max_length=100, default="Active")
    board = models.ForeignKey(Board, on_delete=models.SET_NULL, null=True, related_name="game_instances")
    user1 = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="user1_games")
    user2 = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="user2_games")
    ...
    def __str__(self):
        u1 = self.user1.username if self.user1 else "Deleted"
        u2 = self.user2.username if self.user2 else "Deleted"
        w = self.winner.username if self.winner else "None"
        return f"{u1} vs {u2} -> winner: {w}"